...
public final class ConstantsSaver {
    ...
    private void saveClassToOnBotJava(@NonNull Class<?> clazz) {
        ...
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(constantsFile))) {
            ...
            for (Field field : fields) { saveFieldToOnBotJava(bufferedWriter, field); }
    ...
    private void saveFieldToOnBotJava(
        ...
        try {
            switch (SupportedType.fieldToType(field)) {
                case SPARK_FUN_POSE_2D:
                    line = fieldName + "="
                         + constructPose2DLine((SparkFunOTOS.Pose2D) field.get(null));
                    break;
                case UNSUPPORTED:
        ...
        } catch (IllegalAccessException ignored) { return; }
    ...
    @NonNull private String constructPose2DLine(@NonNull SparkFunOTOS.Pose2D pose2D) {
        return "Pose2D(" + pose2D.x + "," + pose2D.y + "," + pose2D.h + ")";
    }

    private boolean isSavableField(@NonNull Field field) {
